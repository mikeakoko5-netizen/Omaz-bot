...
def build_prompt(market_data, pair, pair_type):
    ...
    prompt += "Respond with ONLY a single-line JSON object (no markdown, no code fences, no line breaks inside string values). "
    ...
    prompt += "- summary and risk_note must be plain short sentences with NO line breaks\n"
    prompt += "- If market is unclear or choppy, set confidence below 55\n"
    prompt += "- Be honest and realistic\n"
    prompt += "- Output raw JSON only, starting with { and ending with }"
    return prompt
...
def extract_json(text):
    ...
    if match:
        ...
        # Remove literal newlines/tabs inside the JSON (common Gemini issue)
...
def analyze():
    try:
        ...
        try:
            ...
            return jsonify({
                "success": False,
                "error": "JSON parse error: " + str(parse_err),
                "raw": text[:500]
            }), 500
